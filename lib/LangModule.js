...
class LangModule extends AbstractModule {
  ...
  addTranslationUtils(req, res, next) {
    ...
    req.translate = (key, data) => {
      if(key.constructor.name === 'AdaptError') {
        return this.translate(lang, `error.${key.code}`, key.data);
      }
      return this.translate(lang, key, data);
    };
    next();
  }
  /**
    ...
    return Object.entries(data).reduce((s, [k,v]) => {
      ...
      if(Array.isArray(v)) {
        ...
        matches.forEach(([replace, data]) => {
          ...
          s = s.replace(replace, v.map(val => attrs.split(',').map(a => val.hasOwnProperty(a) ? val[a] : a)).join(delim));
        });
      }
      return s;
    }, s);
