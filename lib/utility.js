...
class Lang {
  ...
  initialise() {
    const phraseDirs = [...Object.keys(this.app.dependencyloader.dependencies).map(d => path.join(process.cwd(), 'node_modules', d, 'lang')), path.join(process.cwd(), 'lang')];
    const phrases = this.loadPhrases(phraseDirs);
    const p = new Polyglot({ phrases: phrases, warn: (...args) => this.log('warn', ...args) });
    let locale;
